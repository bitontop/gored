...
func (e *Kucoin) ApiKeyRequest(strMethod string, strRequestPath string, mapParams map[string]string) string {
...
func (e *Kucoin) ApiKeyPost(strRequestPath string, mapParams map[string]string) string {
	timestamp := time.Now().UnixNano() / 1e6
	mapParams["tonce"] = strconv.FormatInt(timestamp, 10)
