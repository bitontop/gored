...
const (
	API_URL = "https://belfrics.io/belfrics"
)
...
func (e *Belfries) GetCoinsData() error {
	jsonResponse := &JsonResponse{}
	coinsData := CoinsData{}
...
func (e *Belfries) GetPairsData() error {
	jsonResponse := &JsonResponse{}
	pairsData := PairsData{}
...
func (e *Belfries) OrderBook(p *pair.Pair) (*exchange.Maker, error) {
	jsonResponse := &JsonResponse{}
	orderBook := OrderBook{}
...
/*************** Private API ***************/
func (e *Belfries) UpdateAllBalances() {
...
func (e *Belfries) Withdraw(coin *coin.Coin, quantity float64, addr, tag string) bool {
	if e.API_KEY == "" || e.API_SECRET == "" {
		log.Printf("%s API Key or Secret Key are nil.", e.GetName())
		return false
	}
...
func (e *Belfries) LimitSell(pair *pair.Pair, quantity, rate float64) (*exchange.Order, error) {
	if e.API_KEY == "" || e.API_SECRET == "" {
...
func (e *Belfries) LimitBuy(pair *pair.Pair, quantity, rate float64) (*exchange.Order, error) {
	if e.API_KEY == "" || e.API_SECRET == "" {
		return nil, fmt.Errorf("%s API Key or Secret Key are nil.", e.GetName())
	}
...
func (e *Belfries) OrderStatus(order *exchange.Order) error {
	if e.API_KEY == "" || e.API_SECRET == "" {
...
func (e *Belfries) ListOrders() ([]*exchange.Order, error) {
	return nil, nil
}

func (e *Belfries) CancelOrder(order *exchange.Order) error {
	if e.API_KEY == "" || e.API_SECRET == "" {
		return fmt.Errorf("%s API Key or Secret Key are nil.", e.GetName())
	}
...
func (e *Belfries) CancelAllOrder() error {
	return nil
}
...
func (e *Belfries) ApiKeyGet(strRequestPath string, mapParams map[string]string) string {
...
Step 1: Change Instance Name    (e *<exchange Instance Name>)
Step 2: Create mapParams Depend on API Signature request*/
func (e *Belfries) ApiKeyRequest(strMethod, strRequestPath string, mapParams map[string]string) string {
	strUrl := API_URL + strRequestPath
