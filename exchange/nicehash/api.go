...
const (
	API_URL = "https: //api2.nicehash.com/main"
)
...
func (e *Nicehash) GetCoinsData() error {
	//jsonResponse := &JsonResponse{}
	coinsData := CoinsData{}

	strRequestPath := "/api/v2/public/currencies/"
	strUrl := API_URL + strRequestPath

	jsonResponse := exchange.HttpGetRequest(strUrl, nil)
	if err := json.Unmarshal([]byte(jsonResponse), &coinsData); err != nil {
		return fmt.Errorf("%s Get Coins Json Unmarshal Err: %v %v", e.GetName(), err, jsonResponse)
	}
...
func (e *Nicehash) GetPairsData() error {
	jsonResponse := &JsonResponse{}
	pairsData := PairsData{}
...
func (e *Nicehash) OrderBook(p *pair.Pair) (*exchange.Maker, error) {
	jsonResponse := &JsonResponse{}
	orderBook := OrderBook{}
...
func (e *Nicehash) LoadPublicData(operation *exchange.PublicOperation) error {
	return fmt.Errorf("LoadPublicData :: Operation type invalid: %+v", operation.Type)
}

/*************** Private API ***************/
func (e *Nicehash) DoAccountOperation(operation *exchange.AccountOperation) error {
	return nil
}

func (e *Nicehash) UpdateAllBalances() {
...
func (e *Nicehash) Withdraw(coin *coin.Coin, quantity float64, addr, tag string) bool {
	if e.API_KEY == "" || e.API_SECRET == "" {
		log.Printf("%s API Key or Secret Key are nil.", e.GetName())
		return false
	}
...
func (e *Nicehash) LimitSell(pair *pair.Pair, quantity, rate float64) (*exchange.Order, error) {
	if e.API_KEY == "" || e.API_SECRET == "" {
...
func (e *Nicehash) LimitBuy(pair *pair.Pair, quantity, rate float64) (*exchange.Order, error) {
	if e.API_KEY == "" || e.API_SECRET == "" {
		return nil, fmt.Errorf("%s API Key or Secret Key are nil.", e.GetName())
	}
...
func (e *Nicehash) OrderStatus(order *exchange.Order) error {
	if e.API_KEY == "" || e.API_SECRET == "" {
...
func (e *Nicehash) ListOrders() ([]*exchange.Order, error) {
	return nil, nil
}

func (e *Nicehash) CancelOrder(order *exchange.Order) error {
	if e.API_KEY == "" || e.API_SECRET == "" {
		return fmt.Errorf("%s API Key or Secret Key are nil.", e.GetName())
	}
...
func (e *Nicehash) CancelAllOrder() error {
	return nil
}
...
func (e *Nicehash) ApiKeyGet(strRequestPath string, mapParams map[string]string) string {
...
Step 1: Change Instance Name    (e *<exchange Instance Name>)
Step 2: Create mapParams Depend on API Signature request*/
func (e *Nicehash) ApiKeyRequest(strMethod, strRequestPath string, mapParams map[string]string) string {
	strUrl := API_URL + strRequestPath
